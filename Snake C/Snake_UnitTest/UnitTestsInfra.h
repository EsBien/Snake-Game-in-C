...
{\
	...
	*num_passed = 0; *num_failed = 0;\
	for (struct TESTS_LINKED_LIST_TYPE(SUITE_NAME)* it = tests_anchor; it != NULL; it = it->next) {\
		printf("{\n \"Name\": \"%s\",\n \"RunTime_ms\": %d, \n \"IsPass\":", it->name ,0); fflush(stdout);\
		int is_fail = false;\
		int __INNER_FUNC_CALL_RET_VALUE__ = it->_test_func(_this, &is_fail);\
		if (IN_THROWING_VALUE == __INNER_FUNC_CALL_RET_VALUE__) { is_fail = true; printf("false, \n \"Msg\": \" EXCEPTION THROWN:\ %s\ \" \n" , LAST_EXCEPTION_ERROR_MSG); } \
		if (false != is_fail) { (*num_failed)++; fflush(stdout); }\
		else { (*num_passed)++; printf("true,\n \"Msg\":\"\" \n"); fflush(stdout); }\
		if (it->next == NULL){printf("}");fflush(stdout);}\
...
#define NTEST_ASSERT(x) if (!(x)) {printf("false \n,\"Msg\":\"FAIL: %s is False\" \n ", #x ); *__is_fail__ = true; RETURN;}
